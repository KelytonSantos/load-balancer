...
public:
  ...
  desabilita copia pra evitar que dois objetos fechem o mesmo file descriptor (o delete instrui ao comp proibir a copia de instancias dessa classe)
  */

  // Permite movimentação (Move Semantics)
  /*
    sendo a cópia proibida entao para fazer um retorn socket dentro por exemplo de da func accept oq fazemos é transferir a posse (ownership) do antigo(other) para o novo

    O novo objeto pega o fd_ de other.

    O objeto other tem seu fd_ alterado para -1 (inválido).

    Quando other for destruído, ele não fechará o socket, pois seu fd_ agora é -1.
